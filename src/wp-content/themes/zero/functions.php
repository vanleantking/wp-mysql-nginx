<?php
/**
 * only-me functions and definitions
 *
...
function zero_setup() {
	/*
		...
		* If you're building a theme based on only-me, use a find and replace
	...
	register_nav_menus(
		array(
			'menu-1' => esc_html__( 'Primary', 'zero' ),
		)
	);
...
function zero_scripts() {
	wp_enqueue_style( 'zero-style', get_stylesheet_uri(), array(), _S_VERSION );
	wp_style_add_data( 'zero-style', 'rtl', 'replace' );

	wp_enqueue_script( 'zero-navigation', get_template_directory_uri() . '/js/navigation.js', array(), _S_VERSION, true );
